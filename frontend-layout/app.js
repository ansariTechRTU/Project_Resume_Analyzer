...
function updateDropzoneUI(dropzone, file) {
  ...
  dropzone.innerHTML = `
        ...
        <p class="text-gray-700 font-medium mb-1">
            File uploaded: <span class="text-green-600">${truncateFileName(
              file.name
            )}</span>
        </p>
...
// Handle Analyze Button Click  (BACKEND ENTEGRE HALİ)
async function handleAnalyze() {
  ...
  try {
    ...
    if (jobDescFile) {
      ...
      // paste edilen job description’ı TXT dosyası gibi gönder
      const text = jobDescText.value.trim();
      const blob = new Blob([text], { type: "text/plain" });
      formData.append("jobDescFile", blob, "job-description.txt");
    }
    ...
    // backend: { success: true, id: "<some-id>" }
    const id = json.id;
    if (!id) {
      showNotification(
        "No analysis ID returned from server. Please try again.",
        "error"
      );
      return;
    }
    ...
    showNotification("Network error while analyzing. Please try again.", "error");
  } finally {
